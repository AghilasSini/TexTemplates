...
\tikzstyle{naveqs} = [sensor, text width=6em, fill=black!20, 
    minimum height=4em, rounded corners,visible on=<#1->]
...
\begin{tikzpicture}[scale=1.1]
    \node (naveq) [naveqs={1}] {Waveform Generator};
     	
 	\path (naveq.0)+(-3.8,+0.3) node (psola) [psola={1}] {};
      
    \path (naveq.140)+(-\blockdist,0) node (gyros) [block={1}] {};
    \path (naveq.-150)+(-\blockdist,0) node (accel) [block={1}] {};
...
%\path [draw, ->] (psola.east) -- node [above] {} 
 %       (naveq.west |- psola.east);
    \path (accel.south west)+(+1.0,-0.5) node (IMU) {Front End};
	\path (accel.south west)+(+3.5,-0.5) node (IMU) {Lingustics Specifications};    
    \path (naveq.south west)+(-1.5,-0.5) node (INS) {Speech Synthesis Commune A};
